...
export function RegistrationFormDialog({ isOpen, onClose }: RegistrationFormDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ text: "Вы успешно записались!", type: "success" })
        ...
        if (Array.isArray(data.detail)) {
          ...
          setMessage({ text: messages, type: "error" })
        } else if (typeof data.detail === "string") {
          setMessage({ text: data.detail, type: "error" })
        } else {
          setMessage({ text: "Произошла ошибка при отправке формы", type: "error" })
        }
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[480px] p-0 bg-white shadow-2xl rounded-xl animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 max-h-[90vh]">
        <div className="flex flex-col max-h-[90vh]">
          {/* Header - Fixed */}
          <DialogHeader className="text-center space-y-2 mb-6 px-8 pt-8 flex-shrink-0">
          ...
          {/* Footer - Fixed */}
          <DialogFooter className="mt-6 px-8 pb-8 flex-shrink-0">
            <Button
              type="submit"
              size="lg"
              disabled={loading}
